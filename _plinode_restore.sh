...
FUNC_RESTORE_MENU() {
    ...
    select _file in "${node_backup_arr[@]}" "QUIT" "REBUILD-EI"; do
        case $_file in
        ${node_backup_arr[0]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[0]}"
            BACKUP_FILE="${node_backup_arr[0]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[1]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[1]}"
            BACKUP_FILE="${node_backup_arr[1]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[2]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[2]}"
            BACKUP_FILE="${node_backup_arr[2]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[3]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[3]}"
            BACKUP_FILE="${node_backup_arr[3]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[4]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[4]}"
            BACKUP_FILE="${node_backup_arr[4]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[5]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[5]}"
            BACKUP_FILE="${node_backup_arr[5]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[6]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[6]}"
            BACKUP_FILE="${node_backup_arr[6]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        ${node_backup_arr[7]})
            echo "   RESTORE MENU - Restoring file: ${node_backup_arr[7]}"
            BACKUP_FILE="${node_backup_arr[7]}"
            FUNC_RESTORE_DECRYPT
            break
            ;;
        "QUIT")
